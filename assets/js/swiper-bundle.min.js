		...
		updateActiveIndex: function(e) {
			...
			(p =
				t.virtual && i.virtual.enabled && i.loop
					? c(d)
					: t.slides[d] ? parseInt(t.slides[d].getAttribute('data-swiper-slide-index') || d, 10) : d),
				Object.assign(t, {
					previousSnapIndex: l,
					snapIndex: o,
					previousRealIndex: n,
					...
					for (let e = c; e <= p; e += 1)
						if (e < y || e > E) {
							...
							s.slidesEl
								.querySelectorAll(
									`.${s.params
										.slideClass}[data-swiper-slide-index="${t}"], swiper-slide[data-swiper-slide-index="${t}"]`
								)
								.forEach((e) => {
									e.remove();
								});
			...
			const i =
				...
				r = (e, s) => {
					...
					if (
						(n || l
							? ((n = n || '0'), (l = l || '0'))
							: t.isHorizontal() ? ((n = r), (l = '0')) : ((l = r), (n = '0')),
			...
			ne({
				...
				setTranslate: () => {
					...
					if (
						...
						if (p)
							m.style.transform = `translate3d(0px, ${r / 2 + c.shadowOffset}px, ${-r /
							...
							const e = Math.abs(h) - 90 * Math.floor(Math.abs(h) / 90),
								...
								a = c.shadowScale / t,
								i = c.shadowOffset;
							m.style.transform = `scale3d(${s}, 1, ${a}) translate3d(0px, ${n / 2 + i}px, ${-n /
								2 /
								a}px) rotateX(-90deg)`;
						}
					...
					(s.style.transform = `translate3d(0px,0,${g}px) rotateX(${t.isHorizontal()
						? 0
						: h}deg) rotateY(${t.isHorizontal() ? -h : 0}deg)`),
							...
							if (
								((m =
									p < 0
										...
										: p > 0 ? `calc(${m}px ${a ? '-' : '+'} (-${w * Math.abs(p)}%))` : `${m}px`),
								!t.isHorizontal())
							) {
								const e = h;
								(h = m), (m = e);
